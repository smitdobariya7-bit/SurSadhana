...
const AIGuruPage = () => {
  ...
  const getToken = () => localStorage.getItem('sursadhana_token') || '';
  const getLocalHistoryKey = () => `sursadhana_ai_history_${user?.email || 'guest'}`;
  ...
  const saveHistory = async (nextMessages) => {
    ...
    if (token.startsWith(LOCAL_PREFIX)) {
      localStorage.setItem(getLocalHistoryKey(), JSON.stringify(nextMessages));
      return;
    }

    try {
      await fetch(`${AI_BASE_URL}/api/ai/history`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      if (token.startsWith(LOCAL_PREFIX)) {
        try {
          const localHistory = JSON.parse(localStorage.getItem(getLocalHistoryKey()) || '[]');
          setMessages(Array.isArray(localHistory) ? localHistory : []);
        } catch {
          setMessages([]);
        }
        return;
      ...
      try {
        const res = await fetch(`${AI_BASE_URL}/api/ai/history`, {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      appendMessage({ role: 'assistant', content: 'Error: could not get response from AI.', createdAt: Date.now() });
    } finally {
      setLoading(false);
    }
  };
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-6xl mx-auto">
          ...
          <div className="rounded-2xl border border-white/10 bg-[#0d0f14]/90 shadow-[0_24px_80px_rgba(0,0,0,0.45)] overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-12 min-h-[86vh]">
              ...
              <aside className="lg:col-span-3 p-6 md:p-7 bg-[#0a0c12]">
